...
function ListStudent({
  ...
  const [dropTargetStudent, setDropTargetStudent] = useState(null);
  ...
  const students =
    localSessionStudents.length > 0
      ? localSessionStudents
      : sessionStudents || [];
  const resolvedCourseCode =
    sessionMeta?.course_code || attendance?.course_code;
  ...
  const enrollmentStatusLabel = (value) =>
    enrollmentStatusOptions.find(
      (option) => option.value === normalizeEnrollmentStatus(value),
    )?.label || "Active";
  ...
  const openStudentDetail = async (student) => {
    ...
    if (recordsError && recordsError.message?.includes("does not exist")) {
      const fallback = await supabase
        ...
        .select(
          "*, students!inner(first_name, middle_name, last_name, id_number, course)",
        )
  ...
  const handleEnrollmentStatusChange = async (
    student,
    newStatus,
    skipDropConfirm = false,
  ) => {
    const normalized = normalizeEnrollmentStatus(newStatus);
    const studentIdNumber = Number(student.id_number);
    const currentStatus = normalizeEnrollmentStatus(student.status);

    if (!skipDropConfirm && normalized === "drop" && currentStatus !== "drop") {
      setStatusError("");
      setDropTargetStudent(student);
      setShowDropConfirmModal(true);
      return;
    }

    if (currentStatus === "drop" && normalized !== "drop") {
      setStatusError(
        "Dropped students can only be restored by admin from Manage Student Drop.",
      );
      return;
    }
    ...
    const updatedLocal = (prevStudents || []).map((s) =>
      String(s.id_number) === String(studentIdNumber)
        ? { ...s, status: normalized }
        : s,
    );
    // keep localSessionStudents in sync even if it was empty
    setLocalSessionStudents(
      updatedLocal.length > 0 ? updatedLocal : prevStudents,
    );
    if (
      selectedStudent &&
      String(selectedStudent.id_number) === String(studentIdNumber)
    ) {
    ...
    const statusLabel = enrollmentStatusLabel(normalized);
    const subjectText = attendance?.subject_name
      ? `${attendance.subject_name}${attendance?.section ? ` (${attendance.section})` : ""}`
      : "this class";
    const statusMessageByType = {
      active: `Your enrollment status for ${subjectText} is now ${statusLabel}. Keep up the good work.`,
      warning: `Your enrollment status for ${subjectText} is now ${statusLabel}. Please improve your attendance and coordinate with your faculty adviser.`,
      drop: `Your enrollment status for ${subjectText} is now ${statusLabel}. Please go to the admin office to talk about your status.`,
    };
    const notificationPayload = {
      student_id_number: studentIdNumber,
      title: "Enrollment Status Updated",
      message:
        statusMessageByType[normalized] ||
        `Your enrollment status for ${subjectText} is now ${statusLabel}.`,
    };

    let notificationError = null;
    const { error: notifyRpcError } = await supabase.rpc(
      "notify_enrollment_status_change",
      {
        p_student_id_number: studentIdNumber,
    ...
    if (notificationError) {
      setStatusError(
        `Status updated, but notification was not sent: ${notificationError.message}`,
      );
    }
  ...
  const confirmDropStatusChange = async () => {
    if (!dropTargetStudent) {
      setShowDropConfirmModal(false);
      return;
    }

    await handleEnrollmentStatusChange(dropTargetStudent, "drop", true);
  };
  ...
  const renderStatusBadge = (status) => {
    ...
    return (
      <span
        className={`inline-block rounded-md px-2.5 py-1 text-xs font-semibold capitalize ${styles}`}
      >
        {status}
      </span>
    );
        ...
        <div className="flex items-center justify-between gap-4 border-b border-slate-200 px-5 py-4">
          ...
          {isDetailOpen ? (
            <div className="flex items-center gap-3">
              {normalizeEnrollmentStatus(selectedStudent?.status) ===
                "drop" && (
                <p className="text-xs font-semibold text-rose-600">
                  Locked: only admin can restore dropped students.
                </p>
              )}
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  disabled={
                    String(updatingStatusId) ===
                      String(selectedStudent?.id_number) ||
                    normalizeEnrollmentStatus(selectedStudent?.status) ===
                      "drop"
                  }
          ...
          <>
            ...
            <div className="border-b border-slate-200 px-6 py-4">
              <input
                ...
                onChange={(event) =>
                  setAddStudentsSearchTerm(event.target.value)
                }
                  ...
                  <div className="space-y-3 p-4">
                    {students.map((student) => (
                      <article
                        ...
                        className="cursor-pointer rounded-lg border border-slate-200 bg-white p-4 transition hover:bg-slate-50"
                      >
                  ...
                  <table className="w-full text-left text-sm">
                    <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                      <tr>
                        ...
                        <th className="px-6 py-3 font-semibold select-text cursor-pointer">
                          Status
                        </th>
                      </tr>
                    </thead>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 px-4 py-6">
          <div className="w-full max-w-sm overflow-hidden rounded-xl bg-white shadow-2xl">
            <div className="px-6 py-5">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Set {fullName(dropTargetStudent || {}) || "this student"} to
                Drop status?
              </p>
              <p className="mt-2 text-xs font-medium text-rose-600">
                Once dropped, only admin can restore this student.
              </p>
            </div>
            <div className="flex gap-3 border-t border-slate-200 px-6 py-4">
              ...
              <button
                type="button"
                onClick={confirmDropStatusChange}
                disabled={Boolean(updatingStatusId)}
                className="flex-1 rounded-md bg-rose-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-70"
              >
                {Boolean(updatingStatusId) ? "Proceeding..." : "Proceed"}
              </button>
            </div>
          </div>
        </div>
